...
export default function Services() {
  ...
  const ServiceCard = ({ icon, title, description, dark = false }: any) => (
    ...
    >
      <div
        className={`rounded-3xl p-6 sm:p-8 md:p-12 h-full border transition-all ${
          dark
            ? 'bg-brand-charcoal text-white border-brand-gold/30 shadow-xl'
            : 'bg-[#fffdf9] text-brand-charcoal border-brand-gold/20 shadow-lg'
        }`}
      >
        <div className="text-5xl sm:text-6xl mb-5 sm:mb-6">{icon}</div>
        <h3 className={`text-2xl sm:text-3xl font-bold mb-4 ${dark ? 'text-brand-gold-light' : 'text-brand-charcoal'}`}>{title}</h3>
        <p className={`${dark ? 'text-gray-200' : 'text-gray-600'} text-base sm:text-lg leading-relaxed`}>{description}</p>
      </div>
    </motion.div>
  )
    ...
    <section id="services" className="relative py-16 md:py-24 bg-white overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-bold text-brand-charcoal mb-3 md:mb-4">{t('services_heading')}</h2>
          <p className="text-gray-700 text-base md:text-lg">{t('services_subheading')}</p>
        </motion.div>
